...
y1 = np.fromfile(f'./fermionthesis/BCSEnergiesS/Sbcs{nstates}s{nparticles}p{g1[0]}to{g1[-1]}g{len(g1)}s.dat', sep='\n')
#y2 = np.fromfile(f'./fermionthesis/ExactEnergies/exact{int(nstates)//2}s{nparticles}p{gg[0]}to{gg[-1]}g{len(gg)}s.dat', sep='\n')
y3 = np.fromfile(f'./fermionthesis/EE/ee{int(nstates)-2}s{nparticles}p{g2[0]}to{g2[-1]}g{len(g2)}s.dat', sep='\n')
