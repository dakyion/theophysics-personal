...
int main(){
    ...
    for (int i = 0; i < 2*n; i++)
    {
        ...
        if (i<n/8)
        {
            positions[i] = {x+sc*10,y+sc*40}; 
        }else if(i<n/4){
            positions[i] = {x+sc*30,y+sc*40}; 
        }else if(i<3*n/8){
            positions[i] = {x+sc*20,y+sc*30}; 
        }else if(i<n/2){
            positions[i] = {x+sc*40,y+sc*30}; 
        }else if(i<5*n/8){
            positions[i] = {x+sc*10,y+sc*20}; 
        }else if(i<3*n/4){
            positions[i] = {x+sc*30,y+sc*20}; 
        }else if(i<7*n/8){
            positions[i] = {x+sc*20,y+sc*10}; 
        }else if(i < n){
            positions[i] = {x+sc*40,y+sc*10}; 
        }else if(i < 9*n/8){
            positions[i] = {x+sc*20,y+sc*40}; 
        }else if(i < 5*n/4){
            positions[i] = {x+sc*40,y+sc*40}; 
        }else if(i < 11*n/8){
            positions[i] = {x+sc*10,y+sc*30}; 
        }else if(i < 12*n/8){
            positions[i] = {x+sc*30,y+sc*30}; 
        }else if(i < 13*n/8){
            positions[i] = {x+sc*20,y+sc*20}; 
        }else if(i < 14*n/8){
            positions[i] = {x+sc*40,y+sc*20}; 
        }else if(i < 15*n/8){
            positions[i] = {x+sc*10,y+sc*10}; 
        }else {
            positions[i] = {x+sc*30,y+sc*10}; 
        }
    }
    ...
    std::string filename;
	filename = "sp2000t10d8s9/step0.dat";
    ...
    for (int step = 1; step < 1000; step++)
    {
        ...
        filename = "sp2000t10d8s9/step" + std::to_string(step) + ".dat";
        std::ofstream outfile(filename);
        for(vector<double> p : positions){
            outfile << p[0] << "\t" << p[1] << "\n";
        }
